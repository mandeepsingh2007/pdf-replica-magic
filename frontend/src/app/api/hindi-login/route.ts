...
const AUTH_COOKIE = "tg_hindi_auth";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/hindi/login?error=bad_request", redirectBase(request)), 303);
  }
...
export async function GET(request: Request) {
  return NextResponse.redirect(new URL("/hindi/login", redirectBase(request)), 307);
}
